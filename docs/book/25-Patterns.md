...
回调将代码从行为中解耦，在观察者中经常使用。访问者模式中也使用到了回调，但被称作为“多重派遣”（并非指多重派遣设计模式）。

### 观察者

与其他形式的回调相似的是，观察者内也含有一个钩点，使得你能够改变代码。区别在于观察者的完全动态性，这常用于基于其他对象状态改变而变化的特定情形，同时也是事件管理的基础。观察者使得你能以一种完全动态的方式将调用源与实际被调用的代码解耦。

观察者模式解决了一个较常见的问题：当某个对象的状态发生变化时，如何使得与之关联的一组对象随之更新？这个问题也出现在 SmallTakl 语言的 MVC 中的"模型-视图"层面，又或者是几乎等同的“文档视图体系”中。假使你有一些数据（或是文档）和多个视图，如一个绘图视图和一个文本视图。当你改变数据时，这两个视图必须要知道去更新自己，而这正是观察者要做到的。这是一个相当普遍的问题，所以它的解决方案被添加进了标准库 `java.util` 中。
